...
class Todo {
    ...
    render() {
        ...
        this.listElement.innerHTML = items.map(({id, title, isChecked}) => `
            ...
            <button class="todo-item__delete-button" type="button" aria-label="Delete" title="Delete" data-js-todo-item-delete-button>
                ...
                <path d="M15 5L5 15M5 5L15 15" stroke="#757575" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"/></svg>
            </button>
        </li>
        `).join('')
    ...
    filter() {
        const querryFormatted = this.state.searchQuery.toLocaleLowerCase()

        this.state.filteredItems = this.state.items.filter(({ title }) => {
            const titleFormatted = title.toLocaleLowerCase()

            return titleFormatted.includes(querryFormatted)
        })
        this.render()
    }
...
// @ts-ignore
    ...
    onClick = ({ target }) => {
        if(target.matches(this.selectors.itemDeleteButton)) {
            ...
            itemElement.classList.add(this.stateClasses.isDisapearing)
